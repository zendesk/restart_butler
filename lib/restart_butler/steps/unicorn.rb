class RestartButler::Steps::Unicorn < RestartButler::Steps::Base
  def execute
    if File.exists?("#{butler.root_dir}/log/unicorn.pid")
      ...
      butler.run_command("rvm use 1.9.3-p0 && ./bin/unicorn -c #{butler.root_dir}/interfaces/http/unicorn.rb -E #{butler.opts[:env]["RAILS_ENV"]} -D -p #{opts[:port]} #{butler.root_dir}/interfaces/http/config.ru")
    end
  end
