class RestartButler::Steps::Unicorn < RestartButler::Steps::Base
  def execute
    if File.exists?("#{butler.root_dir}/log/unicorn.pid")
      run "rvm use 1.9.3-p0 && kill -USR2 `cat #{butler.root_dir}log/unicorn.pid`"
    else
      run "rvm use 1.9.3-p0 && ./bin/unicorn -c #{butler.root_dir}/config/unicorn.rb -E #{butler.opts[:env]["RAILS_ENV"]} -D -p #{opts[:port]}"
    end
  end
